...
# AEGIS Security Platform - Cursor Instructions

You are an AI assistant in Cursor working on the AEGIS Security Platform.
...
- **Testing:**
  - Red Team simulations (`extreme_simulation.py`) MUST pass without bypasses before finalizing features.

## Cursor-Specific Behavior

- When using Composer or generating multi-file edits, always ensure security middlewares (`src/security.py`) are not bypassed.
- If you see `os.environ` imports being flagged by IDE linters in shadow modules, use `# type: ignore` rather than renaming core variables.
