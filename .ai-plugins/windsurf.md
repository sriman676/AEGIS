...
# AEGIS Security Platform - Windsurf Instructions

You are an AI assistant in Windsurf working on the AEGIS Security Platform.
...
- When cascading changes across the frontend and backend, always verify that `X-AEGIS-Key` dependencies match.
- Use Windsurf's context awareness to trace execution paths from `kernel.py` to `telemetry.py` to ensure events are properly broadcasted via WebSocket.
